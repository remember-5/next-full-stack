...
type FormErrorValue = string | { message?: string } | undefined;

const FormItemContext = React.createContext<FormItemContextValue>(
  {} as FormItemContextValue,
);

function toErrorArray(value: readonly FormErrorValue[]): FormErrorValue[] {
  return [...value];
}

function getErrorText(error: FormErrorValue) {
...
const useFieldContext = () => {
  ...
  const errors = useStore(store, (state): FormErrorValue[] =>
    toErrorArray(state.meta.errors),
  );
...
function FormErrors({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <form.Subscribe
      selector={(state): FormErrorValue[] => toErrorArray(state.errors)}
    >
      {(errors) => {
